...
import { convertDatesDefault } from '../lib/demo-defaults';
import EndpointSection from './EndpointSection';
import RequestResponsePanel from './RequestResponsePanel';

interface DemoDateConvertProps {
  apiKey: string;
}

export default function DemoDateConvert({ apiKey }: DemoDateConvertProps) {
  const [body, setBody] = useState(
    JSON.stringify(convertDatesDefault, null, 2),
  );
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      const res = await callApi('/dates/convert', {
        method: 'POST',
        body: parsed,
        apiKey,
      });
  ...
  return (
    <EndpointSection
      method='POST'
      path='/api/v1/dates/convert'
      description='Convert dates between Hebrew and Gregorian calendars'
      tier='basic'
    >
      <div className='mt-2 space-y-3'>
        ...
        <div className='flex items-center gap-3'>
          ...
          <button
            onClick={() =>
              setBody(JSON.stringify(convertDatesDefault, null, 2))
            }
