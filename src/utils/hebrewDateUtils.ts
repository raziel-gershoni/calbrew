...
/** Custom flag for Isru Chag events (not defined in @hebcal/core) */
export const ISRU_CHAG_FLAG = 1 << 28;
...
function shouldIncludeEvent(
  ...
  // Isru Chag (custom flag, not from hebcal) — grouped with major holidays
  if (eventFlags & ISRU_CHAG_FLAG) {
    return preferences.majorHolidays;
  }
...
/**
 * Isru Chag definitions: Hebrew day for Israel/Diaspora, month, and festival names
 */
const ISRU_CHAG_DEFS = [
  {
    dayIL: 22,
    dayDiaspora: 23,
    month: months.NISAN,
...
/**
 * Generate Isru Chag events for a given date range.
 * Isru Chag is the day after each of the three pilgrimage festivals.
 * Not generated by @hebcal/core, so we compute them manually.
 */
function generateIsruChagEvents(
  startDate: Date,
  endDate: Date,
  language: string = 'en',
  il: boolean = true,
  ...
  const events: HebrewCalendarEvent[] = [];

  for (let year = startYear; year <= endYear; year++) {
    for (const def of ISRU_CHAG_DEFS) {
      const day = il ? def.dayIL : def.dayDiaspora;
      const hdate = new HDate(day, def.month, year);
      ...
      if (gregDate >= startDate && gregDate <= endDate) {
        const title =
          language === 'he'
            ? `איסרו חג (${def.nameHe})`
            : `Isru Chag (${def.nameEn})`;

        events.push({
          id: `isru-chag-${def.nameEn.toLowerCase()}-${year}`,
          title,
          date: gregDate,
          hebrewDate: hdate,
          type: 'isru_chag',
          flags: ISRU_CHAG_FLAG,
        });
      }
    }
  }
...
function getHebrewEventsForDateRange(
  ...
  try {
    ...
    if (preferences) {
      isruChagEvents = isruChagEvents.filter((event) =>
        shouldIncludeEvent(event.flags, preferences),
      );
    }

    return [...hebcalEvents, ...isruChagEvents].sort(
